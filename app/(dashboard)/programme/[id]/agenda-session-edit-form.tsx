...
import { agendaSessionSchema, type AgendaSessionFormData } from "@/lib/validators/agenda-session";
import { updateAgendaSession as updateAgendaSessionService } from "@/services/agenda-session.service";
...
const STATUS_OPTIONS: AgendaSessionStatus[] = [
  "draft",
  "published",
  "cancelled",
  "completed",
...
async function updateAgendaSession(id: string, data: AgendaSessionFormData) {
  const { error } = await updateAgendaSessionService(id, data as any);
...
export function AgendaSessionEditForm({ event }: { event: AgendaSessionData | null }) {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<AgendaSessionFormData>({
    resolver: zodResolver(agendaSessionSchema) as any,
    defaultValues: event
      ? {
          name: event.name,
  ...
  async function onSubmit(data: AgendaSessionFormData) {
    if (!event) return;
    setIsSubmitting(true);
    try {
      await updateAgendaSession(event.id, data);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="ios-card mx-auto max-w-2xl">
        <CardHeader>
          <CardTitle className="text-headline">Session Details</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(val) =>
                    setValue("status", val as AgendaSessionStatus, {
                      shouldValidate: true,
                    })
                  }
                >
                  ...
                  <SelectContent>
                    {STATUS_OPTIONS.map((s) => (
                      <SelectItem key={s} value={s}>
                        {AGENDA_SESSION_STATUS_CONFIG[s].label}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
