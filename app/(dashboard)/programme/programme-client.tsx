...
import { AgendaSessionDetailModal } from "@/components/features/programme/agenda-session-detail-modal";
import { ScheduleView } from "@/components/features/programme/schedule-view";
...
const STATUS_CHIP_COLORS: Record<AgendaSessionStatus, string> = {
...
export function ProgrammeClient({ sessions, halls = [] }: ProgrammeClientProps) {
  const [statusFilter, setStatusFilter] = useState<string>("all");
  const [hallFilter, setHallFilter] = useState<string>("all");
  const [search, setSearch] = useState("");
  const [calendarMonth, setCalendarMonth] = useState(() => new Date());
  const [selectedDay, setSelectedDay] = useState<Date | null>(null);
  const [modalEvent, setModalEvent] = useState<AgendaSessionItem | null>(null);
  ...
  }, [sessions, statusFilter, hallFilter, search]);
  ...
  if (sessions.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed py-16 text-center">
        <p className="text-headline text-muted-foreground">No sessions found</p>
      ...
      <Tabs defaultValue="grid">
        ...
        <TabsContent value="schedule">
          <ScheduleView
            ...
            onEventClick={(event) => setModalEvent(event as AgendaSessionItem)}
          />
        </TabsContent>
      </Tabs>

      {/* Session Detail Modal */}
      <AgendaSessionDetailModal
        event={modalEvent}
        open={!!modalEvent}
...
function EventCard({ event, onClick }: { event: AgendaSessionItem; onClick: () => void }) {
