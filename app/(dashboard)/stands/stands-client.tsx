...
export function StandsClient({ stands }: { stands: StandItem[] }) {
  ...
  const sorted = useMemo(() => {
    ...
    copy.sort((a, b) => {
      ...
      switch (sortKey) {
        case "stand_number":
          cmp = a.stand_number.localeCompare(b.stand_number, undefined, {
            numeric: true,
          });
          break;
        case "hall_name":
  ...
  function SortIcon({ column }: { column: SortKey }) {
    if (sortKey !== column)
      return (
        <ChevronUp className="ml-1 inline size-3.5 text-muted-foreground/40" />
      );
    return sortDir === "asc" ? (
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        >
          ...
          <SelectContent>
            ...
            {(
              Object.entries(STAND_STATUS_CONFIG) as [
                StandStatus,
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead
                ...
                onClick={() => handleSort("stand_number")}
              >
                Stand # <SortIcon column="stand_number" />
              </TableHead>
              <TableHead
                className="cursor-pointer select-none"
                onClick={() => handleSort("hall_name")}
              >
          ...
          <TableBody>
            {pageItems.map((stand) => {
              ...
              return (
                <TableRow key={stand.id} className="group">
                  <TableCell>
                    <Link
                      href={`/stands/${stand.id}`}
                      className="text-headline text-ios-blue hover:underline"
                    >
                      {stand.stand_number}
                    </Link>
                  </TableCell>
                  ...
                  <TableCell>
                    <Badge
                      className={cn("text-caption-2", statusConfig.color)}
                    >
                      {statusConfig.label}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-right text-footnote">
                    {stand.area_sqm}
                  </TableCell>
